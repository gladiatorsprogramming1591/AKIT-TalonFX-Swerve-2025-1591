...
public class ElevatorIOReal implements ElevatorIO {
  ...
  private final SparkFlex follower =
      new SparkFlex(ElevatorConstants.FOLLOWER_ID, MotorType.kBrushless);
  ...
  private void configureMotors() {
    ...
    zeroTrigger.onFalse(zeroElevatorExternalEncCommand());
  ...
  public void updateInputs(ElevatorIOInputs inputs) {
    ...
    double motorRPS = externalEncoder.getVelocity();
  ...
  public void zeroPosition() {
    externalEncoder.setPosition(0.0);
  }
  ...
  public double inchesToExternalRotations(double inches) {
    return (inches - ElevatorConstants.INITIAL_HEIGHT_INCHES)
        / ElevatorConstants.INCHES_PER_EXTERNAL_ROTATION;
  }
  ...
  public void setPositionExternalRotations(double rotations) {
    if (rotations < getExternalPositionRotations()) {
      controller.setReference(
          ...
          ElevatorConstants.FF_DOWN);
      System.out.println("Lowering Elevator using kSlot1. Reference set to: " + rotations);
    } else {
      controller.setReference(
          rotations, ControlType.kPosition, ClosedLoopSlot.kSlot0, ElevatorConstants.FF_UP);
